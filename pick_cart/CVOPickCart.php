<?php
	...
	class CVOPickCart extends MyDB
	{
		//Define Variables
		...
		{
			$idx = 1;
			$query = "SELECT COUNT(*) as CNT FROM cvo_cart_scan_orders WHERE scan_user = '".$cart."'";
			...
			$query = "exec cvo_pick_cart_process_sp ?, ?, ?, ?";
			$stmt = $dbLink->prepare($query);
		...
		{
			$query = "SELECT TOP 1 '1' as NUMB, op.bin_no FROM cvo_cart_order_parts op 
		...
		function showBinAllocation($dbLink, $cartNo, $binNo)
		{
			...
			if(odbc_num_rows($result) > 0)
			{
				...
				{
					$this->ledON($whichLED, $toPick, $dbLink);
				}				
		...
		*/
		...
		function ledON($ledNo, $pickNum, $dbLink)
		{
			$ledNo .= "1";
			...
			$conf = $this->pullConfig($dbLink);
			$service_url = 'http://'.$conf['cart3_ip'].':'.$conf['cart3_port'].'/'.$ledNo;
		...
		function completeCycle($dbLink, $cartNo)
		{
			...
			$result_get = $dbLink->prepare($query_get);
			$result_get->execute();
			$coData = array();
			while($row_get = $result_get->fetch())
			{
			...
			$result_load_parts = $dbLink->prepare($query_load_parts);
			if($result_load_parts->execute())
			{
				...
				$result_load_orders = $dbLink->prepare($query_load_orders);
				if($result_load_orders->execute())
				{
					...
					$result_update = $dbLink->prepare($query_update_processed);
					if($result_update->execute())
					{
						...
						$result_delete_orders = $dbLink->prepare($query_truncate_orders);
						if($result_delete_orders->execute())
						...
						$result_delete_parts = $dbLink->prepare($query_truncate_parts);
						if($result_delete_parts->execute())
		...
		function pullConfig($dbLink)
		{
			$query = "SELECT * FROM cvo_cart_config";
			$result = $dbLink->prepare($query);
			if($result->execute())
			{
				$row = $result->fetch();
